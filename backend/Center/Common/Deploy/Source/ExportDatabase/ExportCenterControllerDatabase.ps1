...
New-Variable -Name DatabaseName -Scope Script -Value "Gorba_CenterControllers"
...
New-Variable -Name TargetScriptPath -Option ReadOnly -Scope Script -Value (Join-Path (GetScriptDirectory) "../CreateScripts/CreatePersistenceDatabase.sql") # Change this!
...
& $ExportDatabase -DatabaseServer $DatabaseServer -DatabaseUser $DatabaseUser -DatabasePassword $DatabasePassword -DatabaseName $DatabaseName -TargetScriptPath $TargetScriptPath -DatabaseVersion $DatabaseVersion -DatabaseVersionDescription $DatabaseVersionDescription -DatabaseVersionDate $DatabaseVersionDate -UseLocalSecurity: $UseLocalDatabase -DatabaseLogin "gorba_center_controllers" -SkipVersioning -AdditionalRoles @("System.Activities.DurableInstancing.InstanceStoreObservers", "System.Activities.DurableInstancing.InstanceStoreUsers", "System.Activities.DurableInstancing.WorkflowActivationUsers") -AdditionalSchemas @("System.Activities.DurableInstancing")