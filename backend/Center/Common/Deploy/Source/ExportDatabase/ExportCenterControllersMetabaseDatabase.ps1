...
New-Variable -Name DatabaseName -Scope Script -Value "Gorba_CenterControllersMetabase"
...
New-Variable -Name TargetScriptPath -Option ReadOnly -Scope Script -Value (Join-Path (GetScriptDirectory) "../CreateScripts/CreateControllersMetadataDatabase.sql") # Change this!
...
& $ExportDatabase -DatabaseServer $DatabaseServer -DatabaseUser $DatabaseUser -DatabasePassword $DatabasePassword -DatabaseName $DatabaseName -TargetScriptPath $TargetScriptPath -DatabaseVersion $DatabaseVersion -DatabaseVersionDescription $DatabaseVersionDescription -DatabaseVersionDate $DatabaseVersionDate -UseLocalSecurity: $UseLocalDatabase -DatabaseLogin "gorba_center_controllers_metabase" -SkipVersioning -AdditionalRoles @("db_executor")