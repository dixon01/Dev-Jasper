<#
	.SYNOPSIS
	Updates an Azure system based on Center version 2.0 to version 2.2.
	...
	.EXAMPLE
	.\03_UpdateSystem2_0.ps1 -Path path_to_setup_directory -ServiceName icenter-dev -DatabaseConnectionString some_database_connection_string -DatabaseServerName some_database_server_name -DatabaseName some_database_name -StorageConnectionString some_storage_connection_string
#>
param
(
	[Parameter()]
...
begin
{
	...
	function Update-CloudDatabase
	{
		...
		process
		{
			$properties = Convert-ToProperties -ConnectionString $ConnectionString
			...
			Invoke-Sqlcmd -ServerInstance $properties.Server -Database $properties.Database -Username $properties.UserID -Password $properties.Password -Query $query
		...
		process
		{
			...
			foreach ($vm in $Services)
			{
				...
				param($ServiceName, $Slot)
					Write-Message -Message "Starting $($ServiceName).$($Slot)"
	...
	$publishScriptPath = Join-Path $Path "Setup.ps1"
	if (-not(Test-Path $setupScriptPath))
	{
		throw "Path '$($setupScriptPath)' not found"
	}
