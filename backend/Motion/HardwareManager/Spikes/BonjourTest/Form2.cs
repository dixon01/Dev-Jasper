...
namespace DNSServiceBrowser_NET
{
    ...
    public class Form2 : System.Windows.Forms.Form
    {
        ...
        {
            public BrowseData(IServiceInfo serviceInfo)
            {
            ...
            public override bool Equals(object other)
            {
                ...
                if (other != null)
                {
                    result = this.ToString() == other.ToString();
                }
        ...
        //
        public void
        Populate(BrowseData browseData)
        {
            nameField.Text	= browseData.ServiceInfo.Name;
            typeField.Text	= browseData.ServiceInfo.Protocol;
            //domainField.Text	= browseData.Domain;
            hostField.Text	= browseData.ServiceInfo.Addresses[0].ToString();
            portField.Text	= browseData.ServiceInfo.Port.ToString();
            ...
            //
            {
                foreach (var attribute in browseData.ServiceInfo.Attributes)
                {
                    this.serviceTextField.Items.Add(attribute.Name + "=" + attribute.Value);
                }
            }
        }
        ...
        private void typeBox_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                //
                this.query = this.provider.CreateQuery(typeBox.SelectedItem.ToString());
                this.query.ServicesChanged += QueryOnServicesChanged;
                this.query.Start();
            }
            catch
            {
        ...
        private void QueryOnServicesChanged(object sender, EventArgs eventArgs)
        {
            this.UpdateServices();
        }
        ...
        {
            foreach (var serviceInfo in this.query.Services)
            {
                int index = browseList.Items.IndexOf(serviceInfo.Name);
                if (index < 0)
                {
                    browseList.Items.Add(new BrowseData(serviceInfo));
                }
            }

            foreach (BrowseData data in new ArrayList(browseList.Items))
            {
                if (Array.IndexOf(this.query.Services, data.ServiceInfo) < 0)
                {
