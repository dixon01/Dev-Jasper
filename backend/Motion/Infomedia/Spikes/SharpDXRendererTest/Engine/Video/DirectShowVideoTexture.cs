...
namespace Gorba.Motion.Infomedia.SharpDXRendererTest.Engine.Video
{
    ...
    public class DirectShowVideoTexture : ISampleGrabberCB, IDisposable
    {
        ...
        {
            this.Texture = new Texture(graphicsDevice, this.Width, this.Height, 1, Usage.None, Format.A8R8G8B8, Pool.Managed);
        }
        ...
        public void Pause()
        {
            ...
            // Calling pause by itself does not seem to do anything. (DirectShowVideoTexture keeps streaming for some reason).
        ...
        {
            // TODO: how do we know if the device is usable?
            ...
            {
                DataStream stream;
                var rect = this.Texture.LockRectangle(0, LockFlags.None, out stream);
                try
                {
                    ...
                    byte* texturePtr = (byte*)stream.DataPointer;
            ...
            {
                Logger.WarnException("Could not write video into texture", ex);
            }
        ...
        private void HandleGraphEvents()
        {
            ...
            {
                int hr;
                if (eventCode == EventCode.Complete && this.IsLooping)
                {
