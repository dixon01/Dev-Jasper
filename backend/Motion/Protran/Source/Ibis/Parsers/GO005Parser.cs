// --------------------------------------------------------------------------------------------------------------------
// <copyright file="GO005Parser.cs" company="Gorba AG">
...
namespace Gorba.Motion.Protran.Ibis.Parsers
{
    ...
    /// Parser for GO005.
    /// </summary>
    public class GO005Parser : AnswerWithDS120Parser<GO005>
    {
        private bool telegramOnceArrived;

        /// <summary>
        /// Initializes a new instance of the <see cref="GO005Parser"/> class.
        /// </summary>
        public GO005Parser()
            : base(
                "aA",
        ...
        private Status CheckTelegram(byte[] telegram)
        {
            ...
            int length = (chars[3] - '0') * 16;
            length += (chars[4] - '0') * 4;

            if (length != chars.Length - 6)
            {
                // length field is bytes - header(2) - address(1) - length(2) - <CR>(1)
                return Status.IncorrectRecord;
            }
            ...
            if (chars[index] != 0x0D && chars[index] != 0x20 && chars[index] != 0x04)
            {
