...
namespace Luminator.PeripheralProtocol.Core
{
    ...
    public static class Extensions
    {
        ...
        private static readonly Dictionary<PeripheralMessageType, Type> messageTypesReceivedDictionary = new Dictionary<PeripheralMessageType, Type>();

        private static readonly Dictionary<Type, IList<MemberInfo>> propsCache = new Dictionary<Type, IList<MemberInfo>>();

        private static ManualResetEvent peripheralTypesDictionaryCreated = new ManualResetEvent(false);
        ...
        public static Type FindPeripheralMessageType(this PeripheralMessageType peripheralMessageType)
        {
            Type t;
        ...
        /// <exception cref="MissingMethodException">T is an interface not a clas.</exception>
            ...
            foreach (var prop in properties)
            {
                ...
                if (prop.IsClass())
                {
                    ...
                    if (method != null)
                    {
                        ...
                        Debug.WriteLine("\r\nClass " + propType);
                        var size = Marshal.SizeOf(propType);
        ...
        public static List<MemberInfo> GetFieldsAndProperties(Type type, BindingFlags bindingAttr = BindingFlags.Public | BindingFlags.Instance)
        {
        ...
        {
            return GetOrderedPropertiesFor<T>(model.GetType());
        }
        ...
        public static PeripheralHeader GetValidPeripheralHeader(this byte[] buffer, bool headerInNetworkByteOrder)
        {
            try
            {
                ...
                if (buffer.Length >= PeripheralHeader.Size)
                {
                    ...
                    var peripheralHeader = buffer.FromBytes<PeripheralHeader>();
                    ...
                    if (headerInNetworkByteOrder)
                    {
                        ...
                        var messageType = peripheralHeader.MessageType.FindPeripheralMessageType();
                    ...
                    Logger.Trace("Current Peripheral Header Length = {0}, for MessageType {1}", peripheralHeader.Length, peripheralHeader.MessageType);
                    if (peripheralHeader.Length >= PeripheralHeader.Size)
                    {
            ...
            {
                Logger.Error("GetValidPeripheralHeader() general exception Invalid message buffer {0} Details: {1}", ex.Message, ex.InnerException != null ? ex.InnerException.Message : string.Empty);
            }
        ...
        public static bool IsAckMessage(this IPeripheralBaseMessage message)
        {
            return message.IsMessage(PeripheralMessageType.Ack);
        }
        ...
        /// <returns>The <see cref="bool"/>.</returns>
        public static bool IsMessage(this IPeripheralBaseMessage message, PeripheralMessageType messageType)
        {
            return message.Header.MessageType == messageType;
        }

        /// <summary>The is nak message.</summary>
        /// <param name="message">The message.</param>
        /// <returns>The <see cref="bool"/>.</returns>
        public static bool IsNakMessage(this IPeripheralBaseMessage message)
        {
            return message.IsMessage(PeripheralMessageType.Nak);
        }

        /// <summary>The network byte order.</summary>
        /// <param name="header">The header.</param>
        /// <returns>The <see cref="PeripheralHeader"/>.</returns>
        public static PeripheralHeader NetworkToHostByteOrder(this PeripheralHeader header)
        {
        ...
        private static void InitPeripheralTypesDictionary()
        {
            Debug.WriteLine("InitPeripheralTypesDictionary() Enter");
            var assembly = typeof(PeripheralBaseMessage).Assembly;
            var enums = Enum.GetValues(typeof(PeripheralMessageType)) as IEnumerable<PeripheralMessageType>;
            var allMessageTypes = new List<PeripheralMessageType>(enums);
            lock (messageTypesReceivedDictionary)
            {
                try
                {
                    ...
                    foreach (Type classType in assembly.GetTypes().Where(m => m.IsClass))
                    {
                        if (classType.GetProperty("Header") != null)
                        {
                            ...
                            var model = Activator.CreateInstance(classType) as IPeripheralBaseMessage;
                            if (model != null)
                            {
                ...
                finally
                {
                    ...
                    peripheralTypesDictionaryCreated.Set();
                }
            }
        }
