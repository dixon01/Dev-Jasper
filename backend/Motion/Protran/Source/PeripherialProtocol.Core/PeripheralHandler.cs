...
namespace Luminator.PeripheralProtocol.Core
{
    ...
    public abstract class PeripheralHandler : IPeripheralHandler
    {
        ...
        protected PeripheralHandler(PeripheralContext peripheralContext)
            : this()
        {
        ...
        /// <summary>Gets or sets the number of frame bytes to include in the Write buffer stream. Defaults to One. See Constants.Fram</summary>
        public int PeripheralFramingBytesCount
        {
            get
            {
                return this.PeripheralContext?.Config != null ? this.PeripheralContext.Config.DefaultMaxPeripheralFramingBytes : Constants.DefaultMaxPeripheralFramingBytes;
            }
        }
        ...
        public PeripheralContext PeripheralContext { get; set; }
        ...
        public static IPeripheralBaseMessage FindPeripheralMessageModel(IPeripheralHandler handler, byte[] bytes)
        {
            ...
            {
                return handler.Read(memoryStream) as IPeripheralBaseMessage;
            }
        }
        ...
        /// <summary>Find peripheral message class type from the enum message type in header.</summary>
        /// <param name="header">The header.</param>
        /// <returns>The <see cref="Type"/>.</returns>
        public static Type FindPeripheralMessageType(PeripheralHeader header)
        {
            return FindPeripheralMessageType(header.MessageType);
        }
        ...
        /// <returns>The <see cref="Type"/>.</returns>
        public static Type FindPeripheralMessageType(IPeripheralHandler handler, byte[] bytes)
        {
            // read past the framing byte if present in the array
            if (handler == null)
            {
        ...
        /// <returns>The <see cref="Type"/>.</returns>
        public static Type FindPeripheralMessageType(PeripheralMessageType peripheralMessageType)
        {
            return peripheralMessageType.FindPeripheralMessageType();
        }
        ...
        public static byte[] GetPeripheralBytesWriteBuffer(byte[] buffer, bool headerInNetworkByteOrder = true, int totalFrameBytesToInclude = 1)
        {
            ...
            if (headerInNetworkByteOrder)
            {
                ...
                header = header.HostToNetworkByteOrder();
        ...
        {
            Debug.WriteLine(nameof(PeripheralHandler) + ".Dispose");
            if (!this.disposed)
            {
                this.disposed = true;
        ...
        /// <returns>The <see cref="IPeripheralBaseMessage"/>.</returns>
        public abstract IPeripheralBaseMessage ProcessReceivedMessages(PeripheralState state, object message);
        ...
        public object Read(byte[] buffer)
        {
            ...
            try
            {
                ...
                if (length >= PeripheralHeader.Size)
                {
                    ...
                    var peripheralHeader = buffer.GetValidPeripheralHeader(this.IsHeaderNetworkByteOrder);
                    if (peripheralHeader != null)
                    {
                        ...
                        if (length >= peripheralHeader.Length && peripheralHeader.MessageType != PeripheralMessageType.Unknown)
                        {
                            // we have at least the full number of bytes 
                            var type = FindPeripheralMessageType(peripheralHeader.MessageType);
                            if (type != null)
                            {
                                Debug.WriteLine("PeripheralMessageToType Type = " + type);
                                ...
                                if (methodInfo != null)
                                {
                                    ...
                                    Array.Copy(peripheralHeader.ToBytes(), buffer, PeripheralHeader.Size);
        ...
        public object Read(Stream stream)
        {
            ...
            try
            {
                ...
                // test if we have at least the bytes that represent the header's size in bytes in the stream
                if (bytesInStream >= PeripheralHeader.Size)
                {
        ...
        public T Read<T>(Stream stream = null, int readTimeout = 0) where T : class, IPeripheralBaseMessage
        {
        ...
        /// <param name="state">The state.</param>
        /// <returns>The <see cref="object"/>.</returns>
        public object RemoveNextMessage(PeripheralState state)
        {
            ...
            try
            {
                ...
                Logger.Debug("RemoveNextMessage(state) Enter bytesInStream = {0}", bytesInStream);
                if (bytesInStream >= PeripheralHeader.Size)
                {
                    ...
                    if (peripheralMessage != null)
                    {
                        ...
                        var baseMessage = peripheralMessage as IPeripheralBaseMessage;
                        if (baseMessage != null)
                        {
                            try
                            {
        ...
        /// <summary>Save PeripheralVersionsInfo to xml file.</summary>
        ...
        {
            Info("{0} Enter, Save File {1}", nameof(this.SaveVersionInfo), fileName);
            if (peripheralVersions != null)
            {
                var serializer = new XmlSerializer(typeof(PeripheralVersionsInfo));
                using (var file = File.Create(fileName))
                {
                    Debug.WriteLine("Created Peripherial Version file " + fileName);
        ...
        public virtual int Write<T>(T model) where T : class, IPeripheralBaseMessage
        {
            Logger.Info("Write<{0}> to stream", typeof(T));
            return this.Write(model, this.PeripheralContext.Stream);
        }
        ...
        public virtual int Write(byte[] buffer, Stream stream = null)
        {
            ...
            {
                // bogus model
                Logger.Error("Write() Ignored buffer Length < {0}", PeripheralHeader.Size);
                return 0;
            }
        ...
        /// <exception cref="IOException">An I/O error occurs. </exception>
        /// <exception cref="ArgumentNullException"><paramref name="stream"/> is <see langword="null"/>.</exception>
        public virtual int Write<T>(T model, Stream stream) where T : class, IPeripheralBaseMessage
        {
            if (stream == null)
            {
                stream = this.PeripheralContext.Stream;
            ...
            try
            {
                if (stream.CanWrite)
                {
                    ...
                    var writeBuffer = GetPeripheralBytesWriteBuffer(model, this.IsHeaderNetworkByteOrder, extraFrameBytes);
                    Debug.Assert(writeBuffer[0] == Constants.PeripheralFramingByte, "Missing Framing byte as first byte in buffer");
                    if (writeBuffer[0] != Constants.PeripheralFramingByte)
                    {
                        throw new IOException("Invalid Write Buffer, missing first framing byte in buffer!");
                    }
        ...
        /// <returns>The <see cref="int"/>.</returns>
        public virtual int WriteAck(ushort address = Constants.DefaultPeripheralAddress)
        {
            Info("WriteAck");
            return this.Write(new PeripheralAck(address), this.PeripheralContext.Stream);
        }
        ...
        /// <param name="address">The address.</param>
        /// <returns>The <see cref="int"/>.</returns>
        public virtual int WriteNak(ushort address = Constants.DefaultPeripheralAddress)
        {
            Info("WriteNak");
            return this.Write(new PeripheralNak(address), this.PeripheralContext.Stream);
        }
        ...
        /// <summary>The uninitialize medi called on Dispose.</summary>
