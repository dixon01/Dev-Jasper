...
namespace AudioSwitchEchoConsoleApp
{
    ...
    public class AudioSwitchEchoTest
    {
                                ...
                                serialPort.DataReceived += (sender, args) =>
                                    {
                                        if (args.EventType == SerialData.Chars)
                                        {
                                            ...
                                            if (bytesRead > 0)
                                            {
                                                ...
                                                var model = FindPeripheralMessageModel(receivedBytes);
                                                if (model == null)
                                                {
        ...
        private IPeripheralBaseMessage FindPeripheralMessageModel(byte[] bytes)
        {
            try
            {
                return AudioSwitchHandler.FindPeripheralMessageModel(bytes);
            }
        ...
        private Type FindPeripheralMessageType(byte[] bytes)
        {
            try
            {
                return AudioSwitchHandler.FindPeripheralMessageType(bytes);
            }
            catch (ArgumentException exception)
            {
                Console.WriteLine(exception.Message);
        ...
        {
            var bytes = new PeripheralAck().ToBytes();
