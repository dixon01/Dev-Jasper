...
namespace AudioSwitch.Test
{
    ...
    public class AudioSwitchClientTest
    {
        ...
        public static void ClassInit(TestContext context)
        {
            ...
            type.FindPeripheralMessageType();
        }
            ...
            lock (AudioSwitchClientLock)
            {
                ...
                using (var client = new AudioSwitchSerialClient(config))
                {
                    client.PeripheralDataReceived += (sender, args) =>
                        {
                            Debug.WriteLine("Got Peripheral response " + args.Message.Header.MessageType);

                            // we expect the version as a response
                            IPeripheralBaseMessage msg = args.Message;
                            if (msg.IsMessage(PeripheralMessageType.AudioVersionResponse))
                            {
        ...
        public void AudioSwitchSerialClientRS485_RequestVersion()
        {
            lock (AudioSwitchClientLock)
            {
                ...
                using (var client = new AudioSwitchSerialClient(ReadPeripheralAudioConfig()))
                {
                    ...
                    var result = client.WriteVersionRequest();
                    Assert.IsTrue(result, "WriteVersionRequest != true");
        ...
        /// <summary>The audio switch serial client rs485_ open close.</summary>
                    ...
                    client.PeripheralDataReceived += (sender, args) =>
                        {
                            ...
                            if (msg.IsMessage(PeripheralMessageType.GpioStatusResponse))
                            {
        ...
        public void AudioSwitchSerialClientRS485_WriteAudioStausPollRequest()
        {
            lock (AudioSwitchClientLock)
            {
                ...
                using (var client = new AudioSwitchSerialClient(ReadPeripheralAudioConfig()))
                {
                    ...
                    client.PeripheralDataReceived += (sender, args) =>
                        {
                            Debug.WriteLine("Got response " + args.Message.Header.MessageType);
                            IPeripheralBaseMessage msg = args.Message;
                            if (msg.IsMessage(PeripheralMessageType.AudioStatusResponse))
                            {
                                resetEvent.Set();
                            }
                        };
        ...
        public void AudioSwitchSerialClientRS485_WriteEnableAudioBothSpeakers()
        {
            lock (AudioSwitchClientLock)
            {
                ...
                using (var client = new AudioSwitchSerialClient(ReadPeripheralAudioConfig()))
                {
                    ...
                    var ackReceived = client.WriteAudioEnabled(ActiveSpeakerZone.Both);
                    Assert.IsTrue(ackReceived);
                    signaled = resetEvent.WaitOne(Debugger.IsAttached ? 30000 : DefaultTimeout);
                }
        ...
        public void AudioSwitchSerialClientRS485_WriteMultipleAudioStausRequest()
        {
            lock (AudioSwitchClientLock)
            {
                ...
                using (var client = new AudioSwitchSerialClient(ReadPeripheralAudioConfig()))
                {
                    client.PeripheralDataReceived += (sender, args) =>
                        {
                            Debug.WriteLine("*** UNIT TEST Got  Audio Switch response " + args.Message.Header.MessageType);
                            IPeripheralBaseMessage msg = args.Message;
                            ackReceived = msg.IsAckMessage();
                            if (msg.IsMessage(PeripheralMessageType.AudioStatusResponse) || ackReceived)
                            {
                                countdownEvent.Signal();
                            }
                        };
                    ...
                    client.PeripheralDataReceived += (sender, args) =>
                        {
                            Debug.WriteLine("Got response " + args.Message.Header.MessageType);
                            IPeripheralBaseMessage msg = args.Message;
                            if (msg.IsAckMessage() || msg.IsNakMessage())
                            {
                                // older audio hardware did not return the version info but instead an Ack
                                oldVersionFound = true;
                                resetEvent.Set();

                                // Note The older hadrware never responded
                            }
                            else if (msg is PeripheralAudioVersions)
                            {
                                resetEvent.Set();
                            }
                        };
        ...
        public void AudioSwitchSerialClientRS485_WriteSetVolume()
        {
            lock (AudioSwitchClientLock)
            {
                ...
                using (var client = new AudioSwitchSerialClient(ReadPeripheralAudioConfig()))
                {
                    client.PeripheralDataReceived += (sender, args) =>
                        {
                            Debug.WriteLine("Test Got response " + args.Message.Header.MessageType);
                            IPeripheralBaseMessage msg = args.Message;
                            if (msg.IsAckMessage())
                            {
                                m.Set();
                    ...
                    Assert.IsTrue(client.IsComPortOpen);
                    client.WriteSetVolume(10, 20); // made these are made up values
        ...
        public void SimpleSerialPortWrite_AudioDisabled()
        {
            ...
            lock (AudioSwitchClientLock)
            {
                ...
                byte AudioOutputEnable = (byte)PeripheralMessageType.AudioOutputEnable;
