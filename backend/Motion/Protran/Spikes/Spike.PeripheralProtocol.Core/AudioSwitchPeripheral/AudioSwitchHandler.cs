...
namespace Luminator.PeripheralProtocol.Core.AudioSwitchPeripheral
{
    ...
    public sealed class AudioSwitchHandler : PeripheralHandler<PeripheralAudioSwitchMessageType>, IAudioSwitchHandler
    {
        #region Constants
        ...
        public AudioSwitchHandler() : this(null)
        {
        }

        public AudioSwitchHandler(PeripheralContext<PeripheralAudioSwitchMessageType> peripheralContext)
            : base(peripheralContext)
        {
        ...
        /// <exception cref="ConfiguratorException">If errors occurred while de-serializing the file</exception>
        ///         
        public static object FindPeripheralMessageType(byte[] bytes)
        {
            // read past the framing byte if present in the array
            var audioSwitchHandler = new AudioSwitchHandler(new AudioPeripheralContext(new PeripheralConfig(), new MemoryStream(bytes)));
        ...
        /// <returns>The <see cref="IPeripheralBaseMessage"/>.</returns>
          public override IPeripheralBaseMessageType<PeripheralAudioSwitchMessageType> ProcessReceivedMessages(PeripheralState<PeripheralAudioSwitchMessageType> peripheralState, object message)
        {
            var bytes = message?.ToBytes();
            var validChecksum = CheckSumUtil.IsValidChecksum(bytes);
            var baseMessage = message as IPeripheralBaseMessageType<PeripheralAudioSwitchMessageType>;
            if (baseMessage != null)
            {                
                ...
                {
                    case PeripheralAudioSwitchMessageType.AudioVersionResponse:
                        ...
                        if (validChecksum && peripheralAudioVersions != null)
                        {
                            ...
                            this.SaveVersionInfo(peripheralAudioVersions.VersionsInfo);
                        }
                        break;

                    case PeripheralAudioSwitchMessageType.GpioStatusResponse:
                        this.WriteResponse(validChecksum);
                    ...
                    case PeripheralAudioSwitchMessageType.AudioStatusResponse:
        ...
        private void VolumeSettingsMessageMediHandler(object sender, MessageEventArgs<VolumeSettingsMessage> e)
        {
            ...
            if (volumeSettingsMessage != null && volumeSettingsMessage.MessageAction == MessageActions.Set)
            {
                ...
                if (currentInteriorVolume != VolumeSettingsMessage.Ignored && currentExteriorVolume != VolumeSettingsMessage.Ignored)
                {
                    // fire event to have the Audio switch serial client now apply the new current volume levels.
                    this.FireVolumeChangeRequestedEvent(new VolumeChangeRequest(currentInteriorVolume, currentInteriorVolume));
                }
            }
        }
