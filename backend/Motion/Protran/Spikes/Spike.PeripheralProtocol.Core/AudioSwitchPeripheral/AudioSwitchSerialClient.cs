...
namespace Luminator.PeripheralProtocol.Core.AudioSwitchPeripheral
{
    ...
    public class AudioSwitchSerialClient : PeripheralSerialClient<AudioSwitchHandler, PeripheralAudioSwitchMessageType>, IAudioSwitchSerialClient
    {
        ...
        public event EventHandler<PeripheralAudioSwitchDataReceivedEventArgs> PeripheralAudioSwitchDataReceived;
        #endregion
        ...
        {
            Logger.Info("WriteAudioConfig Enter");
            return this.Write(audioConfig, false);
        }
        ...
        /// <returns>The <see cref="int" />Bytes written</returns>
        /// <exception cref="IOException">An I/O error occurs. </exception>
        public int WriteAudioStausRequest()
        {
            Logger.Info("WriteAudioStausRequestEnter");
            return this.Write(new PeripheralRequestStatus(), false);
        }
        ...
        public int WriteSetVolume(byte interiorVolume, byte exteriorVolume)
        {
            Logger.Info("Setting new Volume levels Interior={0}, Exterior={1}", interiorVolume, exteriorVolume);
            return this.Write(new PeripheralSetVolume(interiorVolume, exteriorVolume), true);
        }
            ...
            lock (updateLock)
            {
                ...
                try
                {
                    using (var stream = File.OpenText(fileName))
                    {
                        ...
                        {
                            var bytes = s.Length;
                            // TODO create a ImageRecord and write to the peripherial
                        }
                    }
                }
                catch (Exception ex)
                {
                    Logger.Error("UpdateImage Excetion {0}", ex.Message);
                }
                finally
                {
        ...
        private void WriteAudioZonesOutput(AudioPlaybackEvent audioPlaybackEvent)
        {
            ...
            if (Enum.TryParse(value.ToString(), out audioZone))
            {
                ...
                this.WriteAudioEnabled(audioZone);
            }
            else
            {
        ...
        public PeripheralVersionsInfo WriteVersionRequest()
        {
            // Request the Version information from the peripheral
            
            var versonResponseMessage = base.WriteRequestVersion(new PeripheralAudioSwitchRequestVersion());
            if (versonResponseMessage != null)
            {
                var audioVersionResponse = versonResponseMessage as PeripheralAudioVersions;
                if (audioVersionResponse != null)
                {
                    return audioVersionResponse.VersionsInfo;
                }
            }
        ...
        private void SubscribeAudioSwitchEvents()
        {
            ...
            base.PeripheralDataReceived += (sender, args) =>
                {
                    var handler = this.PeripheralAudioSwitchDataReceived;
                    handler?.Invoke(sender, new PeripheralAudioSwitchDataReceivedEventArgs(args.Message));
                };
        }
