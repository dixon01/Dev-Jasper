...
namespace ARSoft.Tools.Net.Dns
{
    ...
    {
        private static readonly Logger Logger = LogManager.GetLogger("ARSoft.Tools.Net.Dns.DnsClient");
            ...
            for (int i = 0; i < endpointInfos.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (resultData != null)
                    {
                        ...
                        {
                            Logger.Debug(ex, "Error on dns query");
                            continue;
                        }
        ...
        private void PrepareAndBindUdpSocket(DnsClientEndpointInfo endpointInfo, System.Net.Sockets.Socket udpClient)
        {
            ...
            {
                udpClient.Connect(new IPEndPoint(endpointInfo.ServerAddress, _port));
            }
        }
