		...
		private SpfQualifier CheckMechanism(SpfMechanism mechanism, IPAddress ip, string sender, string domain)
		{
			...
			switch (mechanism.Type)
			{
			    ...
			    case SpfMechanismType.A:
			        bool? isAMatch = IsIpMatch(
			            String.IsNullOrEmpty(mechanism.Domain) ? domain : mechanism.Domain,
			            ip,
			            mechanism.Prefix,
			            mechanism.Prefix6);
			        if (!isAMatch.HasValue) return SpfQualifier.TempError;
			    ...
			    case SpfMechanismType.Mx:
			        dnsMessage =
			            ResolveDns(
			                ExpandDomain(String.IsNullOrEmpty(mechanism.Domain) ? domain : mechanism.Domain, ip, sender, domain),
			                RecordType.Mx);
			        if ((dnsMessage == null)
			            || ((dnsMessage.ReturnCode != ReturnCode.NoError) && (dnsMessage.ReturnCode != ReturnCode.NxDomain))) return SpfQualifier.TempError;

			        int mxCheckedCount = 0;

			        foreach (MxRecord mxRecord in dnsMessage.AnswerRecords.OfType<MxRecord>())
			        {
			            if (++mxCheckedCount == 10) break;

			            bool? isMxMatch = IsIpMatch(mxRecord.ExchangeDomainName, ip, mechanism.Prefix, mechanism.Prefix6);
			            if (!isMxMatch.HasValue) return SpfQualifier.TempError;
			    ...
			    case SpfMechanismType.Ip6:
			        ...
			        try
			        {
			            compareAddress = IPAddress.Parse(mechanism.Domain);
			        }
			        ...
			        if (ip.AddressFamily != compareAddress.AddressFamily) return SpfQualifier.None;

			        if (mechanism.Prefix.HasValue)
			        {
			            if ((mechanism.Prefix.Value < 0)
			                || (mechanism.Prefix.Value
			                    > (compareAddress.AddressFamily == AddressFamily.InterNetworkV6 ? 128 : 32))) return SpfQualifier.PermError;

			            if (
			                ip.GetNetworkAddress(mechanism.Prefix.Value)
			    ...
			    case SpfMechanismType.Ptr:
			        ...
			        {
			            if (++ptrCheckedCount == 10) break;

			            bool? isPtrMatch = IsIpMatch(ptrRecord.PointerDomainName, ip, 0, 0);
			            if (isPtrMatch.HasValue && isPtrMatch.Value)
			            {
			                if (ptrRecord.PointerDomainName.Equals(ptrCompareName, StringComparison.InvariantCultureIgnoreCase)
			                    || (ptrRecord.PointerDomainName.EndsWith(
			                        "." + ptrCompareName, StringComparison.InvariantCultureIgnoreCase))) return mechanism.Qualifier;
			            }
			        }
			        break;

			    case SpfMechanismType.Exist:
			        if (String.IsNullOrEmpty(mechanism.Domain)) return SpfQualifier.PermError;
			    ...
			    case SpfMechanismType.Include:
			        if (String.IsNullOrEmpty(mechanism.Domain)
			            || (mechanism.Domain.Equals(domain, StringComparison.InvariantCultureIgnoreCase))) return SpfQualifier.PermError;
