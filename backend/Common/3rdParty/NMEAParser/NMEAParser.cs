...
// --------------------------------------------------------------------------------------------------------------------
// <copyright file="NMEAParser.cs" company="carpintero48, codeporject.com">
...
namespace NMEA
{
    ...
    /// Talkers identifiers
    /// </summary>
    [SuppressMessage("StyleCop.CSharp.LayoutRules", "SA1506:ElementDocumentationHeadersMustNotBeFollowedByBlankLine",
        Justification = "Reviewed. Suppression is OK here.")]
    [SuppressMessage("StyleCopPlus.StyleCopPlusRules", "SP2000:CodeLineMustNotEndWithWhitespace",
        Justification = "Reviewed. Suppression is OK here.")]
    ...
    public struct GeographicDimension
    {
        ...
        public override string ToString()
        {
            ...
            int minutes = (int)Math.Floor((Angle - degree) * 60.0F);
            float seconds = (Angle - degree) * 3600F - minutes * 60.0F;

            return string.Format(CultureInfo.InvariantCulture, "{0}°{1}\'{2:F04}\" {3}", degree, minutes, seconds, Cardinal);
        }

        public GeographicDimension(float angle, Cardinals cardinal)
        {
            Angle = angle;
            Cardinal = cardinal;
        }

        public static float AsSignedAngle(GeographicDimension dimension)
        {
            float result = dimension.Angle;
    ...
    public static class NMEAParser
    {
        ...
        [SuppressMessage("StyleCop.CSharp.SpacingRules", "SA1013:ClosingCurlyBracketsMustBeSpacedCorrectly",
            Justification = "Reviewed. Suppression is OK here.")]
        ...
        private static Dictionary<SentenceIdentifiers, string> SentencesFormats =
            new Dictionary<SentenceIdentifiers, string>() { { SentenceIdentifiers.AAM, "A=Arrival circled entered|V=V,A=Perpendicular passed at way point|V=V,x.x,N=nm|K=km,c--c" },
                                                            ...
                                                            { SentenceIdentifiers.ASD, string.Empty },
                                                            ...
                                                            { SentenceIdentifiers.DSC, string.Empty },
                                                            { SentenceIdentifiers.DSE, string.Empty },
                                                            { SentenceIdentifiers.DSI, string.Empty },
                                                            { SentenceIdentifiers.DSR, string.Empty },
                                                            ...
                                                            { SentenceIdentifiers.GGA, "hhmmss.sss,llll.ll,a,yyyyy.yy,a,0=Fix not availible|1=GPS fix|2=DGPS fix,xx,x.x,x.x,M,x.x,M,x.x,xxxx" },
                                                            ...
                                                            { SentenceIdentifiers.MSS, string.Empty },
                                                            { SentenceIdentifiers.MTW, "x.x,C=C|K=K|F=F" },
                                                            { SentenceIdentifiers.MWD, string.Empty },
                                                            ...
                                                            { SentenceIdentifiers.RMC, "hhmmss.sss,A=Valid|V=Invalid,llll.ll,N=North|S=South,yyyyy.yy,E=East|W=West,x.x,x.x,ddmmyy,x.x,a,a" },
                                                            ...
                                                            { SentenceIdentifiers.WDC, string.Empty },
                                                            { SentenceIdentifiers.WDR, string.Empty },
        ...
        [SuppressMessage("StyleCop.CSharp.ReadabilityRules", "SA1123:DoNotPlaceRegionsWithinElements",
            Justification = "Reviewed. Suppression is OK here.")]
        [SuppressMessage("StyleCop.CSharp.SpacingRules", "SA1012:OpeningCurlyBracketsMustBeSpacedCorrectly",
            Justification = "Reviewed. Suppression is OK here.")]
        [SuppressMessage("StyleCop.CSharp.SpacingRules", "SA1013:ClosingCurlyBracketsMustBeSpacedCorrectly",
            Justification = "Reviewed. Suppression is OK here.")]
            ...
            new Dictionary<ManufacturerCodes, Dictionary<string, string>>()
            {
                ...
                #region Martech Inc.
                {
                    ...
                    new Dictionary<string, string>()
                    {
                        ...
                        {"101", string.Empty }, // Hot restart: use all available data in the NV Store
                        {"102", string.Empty }, // Warm restart: don't use Ephemeris at re-start
                        {"103", string.Empty }, // Cold restart: don't use Time, Position, Almanacs and Ephemeris data at re-start
                        {"104", string.Empty }, // Full cold restart: it's essentially a Cold Restart, but additionaly clear system/user config at re-start
                        ...
                        {"420", string.Empty}, // API_Query_Pwr_Sav_Mode, return: PMTK520
                        {"490", string.Empty}, // API_Get_Flash_User_Option, return: PMTK590
                        {"520", "0=False|1=True"}, // Power saving operation mode
                        {"590", "x,x,x,x,x,x,x,x,x,x,x,x,0=Disable|1=RTCM|2=SBAS,x"},
                        {"605", string.Empty}, // Query the firmware release information, return: PMTK705
        ...
        private static Dictionary<string, FormatterDelegate> formatters = new Dictionary<string, FormatterDelegate>()
        {
            ...
            { "x.x", x => ((float)x).ToString("F04", CultureInfo.InvariantCulture).TrimEnd(new char[] {'0'}) },
            { "c--c", x => x.ToString() },
            { "llll.ll", x => FormatLatitude((float)x) },
            { "yyyyy.yy", x => FormatLongitude((float)x) },
            ...
            { "dddmm.mmm", x => FormatDegree((float)x) }
        };
        ...
        /// <return>NMEA0183 sentence as string</return>
        ...
        [SuppressMessage("StyleCop.CSharp.MaintainabilityRules", "SA1407:ArithmeticExpressionsMustDeclarePrecedence",
            Justification = "Reviewed. Suppression is OK here.")]
        private static string FormatLatitude(float x)
        {
            StringBuilder sb = new StringBuilder();

            float degrees = (float)Math.Floor(x);
            float minutes = (float)Math.Floor((x - degrees) * 60.0F);
            float seconds = (x - degrees) * 3600 - minutes * 60.0F;
        ...
        [SuppressMessage("StyleCop.CSharp.MaintainabilityRules", "SA1407:ArithmeticExpressionsMustDeclarePrecedence",
            Justification = "Reviewed. Suppression is OK here.")]
        private static string FormatLongitude(float x)
        {
            StringBuilder sb = new StringBuilder();
        ...
        [SuppressMessage("StyleCop.CSharp.MaintainabilityRules", "SA1407:ArithmeticExpressionsMustDeclarePrecedence",
            Justification = "Reviewed. Suppression is OK here.")]
        private static string FormatDegree(float x)
        {
            StringBuilder sb = new StringBuilder();
        ...
        public static NMEASentence Parse(string sourceString)
        {
            if (!sourceString.EndsWith(SentenceEndDelimiter))
            {
                throw new ArgumentException(
                    string.Format(
                        "{0} sentence must ends with valid sentence end delimiter \"{1}\"",
                        StandartID,
                        @"\r\n"));
            }
            ...
            if (source.Length > MaxSentenceLength)
            {
                throw new ArgumentException(
                    string.Format(
                        "Specified string \"{0}\" length exceeds {0} characters",
                        source.Length,
                        MaxSentenceLength));
            }
            ...
            if (!source.StartsWith(SentenceStartDelimiter.ToString()))
            {
                throw new ArgumentException(
                    string.Format(
                        "{0} sentence must starts with valid sentence start delimiter \"{1}\"",
                        StandartID,
                        SentenceStartDelimiter));
            }
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(checkSumString))
                {
                    ...
                    if (byte.TryParse(checkSumString, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out declaredCheckSum))
                    {
        ...
        {
            float temp = float.Parse(token, CultureInfo.InvariantCulture);
        ...
        [SuppressMessage("StyleCop.CSharp.MaintainabilityRules", "SA1407:ArithmeticExpressionsMustDeclarePrecedence",
            Justification = "Reviewed. Suppression is OK here.")]
        private static float ParseLatitude(string token)
        {
            float temp = float.Parse(token, CultureInfo.InvariantCulture);

            float degree = (int)((int)temp / 100.0F);
            float minutes = (int)temp - (degree * 100.0F);
            float seconds = (temp - (int)temp) * 60.0F;

            return degree + minutes / 60.0F + seconds / 3600.0F;
        }

        [SuppressMessage("StyleCop.CSharp.MaintainabilityRules", "SA1407:ArithmeticExpressionsMustDeclarePrecedence",
            Justification = "Reviewed. Suppression is OK here.")]
        private static float ParseLongitude(string token)
        {
            float temp = float.Parse(token, CultureInfo.InvariantCulture);
        ...
        [SuppressMessage("StyleCop.CSharp.MaintainabilityRules", "SA1407:ArithmeticExpressionsMustDeclarePrecedence",
            Justification = "Reviewed. Suppression is OK here.")]
        private static float ParseCommonDegrees(string token)
        {
            float temp = float.Parse(token, CultureInfo.InvariantCulture);
        ...
        public static float Bend2MpS(float speedBends)
        {
            return speedBends * 0.5144F;
        }
        ...
        public static float NM2Km(float nMiles)
        {
            return nMiles * 1.852F;
        }
        ...
        public static float Ft2M(float feets)
        {
            return feets * 0.3048F;
        }
